...
class vediosDataTable extends DataTable
{
    ...
    public function dataTable(DataTables $dataTables, $query)
    {
        return datatables($query)
            ->addColumn('actions', 'admin.vedios.buttons.actions')

            ->addColumn('vedio', '<a href="{{ it()->url($vedio) }}" target="_blank"><i class="fa fa-download fa-2x"></i></a>')
   		...
   		->addColumn('created_at', '{{ date("Y-m-d H:i:s",strtotime($created_at)) }}')   		->addColumn('updated_at', '{{ date("Y-m-d H:i:s",strtotime($updated_at)) }}')            ->addColumn('checkbox', '<div  class="icheck-danger d-inline ml-2">
            ...
            ->rawColumns(['checkbox','actions',"vedio",]);
    }
    ...
    {
        return vedio::query()->select("vedio.*");
